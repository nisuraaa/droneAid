  ...
  const changestatus = async (uuid, status) => {
    ...
    const response = await fetch(window.config.choreoApiUrl + '/drone/changestatus/', {
      ...
      .then((data) => {
        setDrones((prevDrones) => {
          const updatedDrones = prevDrones.map((drone) =>
            drone.uuid === uuid ? { ...drone, status: status } : drone
          );
          return updatedDrones;
        });
        setDroneData((prevDroneData) => {
          const updatedDroneData = { ...prevDroneData, status: status };
          return updatedDroneData;
        });
      })
  }
                      ...
                      <Flex flexDirection={'row'} gap={'10px'}>
                        <Flex flexDirection={'column'} gap={'10px'} width={'50%'}>
                          ...
                          <Flex flexDirection={'column'} border={'1px solid #E5E7EB'} borderRadius={'10px'} flex={1} gap={'10px'} p={'30px'}>
                            ...
                            <Button onClick={() => {
                              changestatus(droneData?.uuid, 'idle')
                            }}
                              isDisabled={droneData?.status === 'returning' || droneData?.status === 'loading' || droneData?.status === 'charging'
                                ? false : true}
                            >
                              Idle
                            </Button>
                            <Button onClick={() => {
                              changestatus(droneData?.uuid, 'delivered')
                            }}
                              isDisabled={droneData?.status === 'delivering'
                                ? false : true}
                            >
                              Delivered
                            </Button>
                            <Button onClick={() => {
                              changestatus(droneData?.uuid, 'returning')
                            }}
                              isDisabled={droneData?.status === 'delivered' ?
                                false : true}
                            >
                              Returning
                            </Button>
